...
module AppLog = (val Log.withNamespace("Onta.App"));
module SdlLog = (val Log.withNamespace("Onta.SDL2"));
  ...
  let appLoop = () => {
    ...
    if (appInstance.isFirstRender
      ...
      if (appInstance.idleCount > 0) {
        Log.info("Upshifting into active state.");
      };
      ...
      if (appInstance.idleCount === framesToIdle) {
        Log.info("Downshifting into idle state...");
        appInstance.onIdle();
      };
